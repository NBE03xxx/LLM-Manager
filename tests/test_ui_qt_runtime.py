...
class QtRuntimeTests(unittest.TestCase):
    ...
    def test_minimal_window_constructs_and_switches_language(self) -> None:
        ...
        try:
            ...
            host_label = window.findChild(QLabel, "selected-host")
            language_label = window.findChild(QLabel, "language-label")
            profile = window.findChild(QComboBox, "profile-selector")
            profile_label = window.findChild(QLabel, "profile-label")
            self.assertIsNotNone(diagnose)
            self.assertIsNotNone(status)
            self.assertIsNotNone(language)
            self.assertIsNotNone(host_label)
            self.assertIsNotNone(language_label)
            self.assertIsNotNone(profile_label)
            self.assertEqual(diagnose.text(), "Diagnose")
            self.assertEqual(diagnose.accessibleName(), "Diagnose")
            self.assertEqual(host.accessibleName(), "Hosts")
            self.assertEqual(host.accessibleDescription(), "Hosts")
            self.assertEqual(host_label.text(), "Hosts")
            self.assertIs(host_label.buddy(), host)
            self.assertEqual(language_label.text(), "Language")
            self.assertIs(language_label.buddy(), language)
            self.assertEqual(language.accessibleDescription(), "Language")
            self.assertEqual(profile_label.text(), "Optimization profile")
            self.assertIs(profile_label.buddy(), profile)
            self.assertEqual(profile.accessibleDescription(), "Optimization profile")
            language.setCurrentIndex(1)
            self.application.processEvents()
            self.assertEqual(diagnose.text(), "診断する")
            self.assertEqual(diagnose.accessibleName(), "診断する")
            self.assertEqual(host.accessibleName(), "ホスト")
            self.assertEqual(host.accessibleDescription(), "ホスト")
            self.assertEqual(host_label.text(), "ホスト")
            self.assertEqual(language_label.text(), "言語")
            self.assertEqual(language.accessibleDescription(), "言語")
            self.assertEqual(profile_label.text(), "最適化プロファイル")
            self.assertEqual(profile.accessibleDescription(), "最適化プロファイル")
