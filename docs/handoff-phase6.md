...
- **2026-09-12 新candidate remote helper lifecycle完了**: b15a984由来remote debで、8854232由来`0.1.0~dev0`からのupgrade、reinstall、remove、fresh install、purge、readiness metadata、root owner/mode、private runtime、local GUI/PolicyKit非混入をUbuntu一時snapshot内で確認。全境界で保全path不変、snapshot内cleanupとrunning復元後にpackage/manual/pathがbaseline完全一致。一時snapshotとupgrade用旧debは削除、既存snapshot保持、両VM running。詳細: `docs/validation/phase6-remote-helper-candidate-lifecycle-2026-09-12.md`。次は新candidateの別マシン間SSH切断、長時間Agent、accessibility、SBOM/license等の残Gate。UNRELEASED維持、現在・次ともPhase 6。

- **2026-09-12 新candidate Debian lifecycle/実display完了**: b15a984由来local debでfresh install/reinstall/remove/再fresh install/purge、GNOME menu通常起動、UID 1000隔離argv、日英keyboard切替、Alt+F4通常終了を確認。追加12件だけを明示purgeしpackage/manual/既存設定等のbaseline完全一致、転送deb削除済み。両VM running維持。詳細: `docs/validation/phase6-debian-candidate-display-2026-09-12.md`。次は新candidate remote helper/SSH等の残Gate。Debian旧版upgradeと最終artifact反復は未完了。UNRELEASED維持、現在・次ともPhase 6。
...
- **2026-09-11 installed SSH GUI Gate完了（最新、snapshot復元済み）**: 同candidate両deb＋公式OpenCode 1.18.30をUbuntu一時snapshotへ導入。Wayland Results GUI→同VMの通常SSHで、Secret Service/remote root dual backup、実validator、commitと不正JSON自動rollback、helper応答喪失注入後のimmutable result照合が成功。各operation mutation再送なし。初回画像はHostsのままだったため可視assertを加えて別IDで取り直し、採用画像を目視確認。productionでも残っていた英日「Sandbox」結果文言を修正。全799件（760成功・39 expected skip）、Ubuntu Qt/SSH/i18n42件成功。証拠回収後running snapshot復元、package/manual/path hashが開始値と完全一致、一時snapshotのみ削除済み。両VM running、Debian未変更。詳細: `docs/validation/phase6-ssh-gui-installed-2026-09-11.md`。別マシン間の物理切断、推奨から人手review全経路、最終artifact反復は未完了。変更は未コミット。現在・次ともPhase 6。

- **2026-09-11 再開preflight（最新）**: 前回の未コミット変更を保持。host全798件（759成功・39 expected skip）、Ubuntu Qt/SSH構成35件が成功。候補deb両hashとshell/desktop/diff検査成功。両VM runningを維持、Ubuntu Wayland active/unlocked、Secret Service利用可能・default collection unlocked。通常SSH正常だがremote helperとOpenCode候補は未導入、設定3 target不在、sudoは対話認証が必要。実OpenCodeを含むGate用の一時導入範囲を利用者に確認する段階で、snapshot/package/key/実設定は変更していない。詳細: `docs/validation/phase6-resume-preflight-2026-09-11.md`。変更は未コミット。現在・次ともPhase 6。

- **2026-09-10 SSH切断照合Qt回帰（今回の最新slice）**: 使用量制約のためテスト・文書だけを更新。実Qt worker＋SSH coordinator＋一時journalに切断fixtureを注入し、commit、rollback結果照合、Apply結果不明、rollback結果不明の4ケースでGUI/journal一致とmutation再送なしを確認。Ubuntu stock Qt offscreenで関連8 test成功、host coordinator 6 test成功。VMのpackage/実設定/状態は変更せず、転送した一時treeは削除済み。通常SSHは正常だがremote helper未導入、OpenCode候補3 pathと設定3 targetは不在。実回線切断やOpenCode実行のGateではなく、最終artifact実GUI SSH Gateは未完了。詳細: `docs/validation/phase6-ssh-qt-reconciliation-2026-09-10.md`。全suite再実行は省略。現在・次ともPhase 6。
