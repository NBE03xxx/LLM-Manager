...
- 2026-09-19、[GitHub Release公開後再取得検証](validation/phase6-public-release-verification-2026-09-19.md)を完了。
  [LLM Manager 0.1.0](https://github.com/NBE03xxx/LLM-Manager/releases/tag/v0.1.0)へ11 assetを公開し、
  GitHub APIの集合／size／state／digestを正本と照合。認証なし公開URLから別directoryへ11/11を
  再取得し、checksum 9/9、公開鍵だけの隔離GPGでmanifest／tag署名、両deb verifier、3環境証拠
  verifier、signed tag peeled target `5b7d4de03e495fe630deab952de043f945a22bd7`を再検証した。
  進捗44/44、100.0%。Phase 6 MVP release checklistは完了。公開済み`v0.1.0` assetを置換しない。
- 2026-09-19、[signed tag `v0.1.0`](validation/phase6-signed-tag-2026-09-19.md)を作成・検証。
  ...
  originへpushし、remote object／peeled targetも一致。この時点の進捗は43/44、97.7%だった。
...
2026-09-19現在は44/44件、**100.0%（公開チェックリスト項目数ベース）**。
...
signed tag、GitHub Release公開、公開後再取得検証はすべて完了。
...
1. Phase 6 MVP release `v0.1.0`は44/44項目完了。公開済みtag／assetを上書き・置換しない。
   追加修正が必要な場合は新versionと新しいsource commitからrelease工程を開始する。
...
3. release専用署名鍵、target distribution、署名者、公開先は確定済み。final source metadataで
   ...
   秘密鍵を自動生成・推測選択しない。
   今後のrelease公開承認を、今回の承認や通常のcommit/push承認と同一視しない。
