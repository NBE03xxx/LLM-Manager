...
backup一覧はlocal manifest、remote retention record、直近local/remote retention evidence、検証済み削除resultをcopyごとにread-only集約する。local retention resultも削除前後の一覧差分、host、評価時刻、removed/remaining、partial/failed/unknown、errorをhashで束縛し、0700 directory内の0600 immutable fileへ保存する。片側削除失敗後のread-only reconciliation resultはsource deletion result hash、manifest hash、host/fingerprint、観測時刻、両copy presence/stateをcanonical hashで束縛し、0700/0600 immutable storeへ保存する。協調削除はremote mutation前にcanonical manifestをdeletion request hash名の0700/0600 immutable evidenceへ保存し、失敗時は削除を開始しない。このevidenceはmanifest identity/path/item metadataだけで、backup本文や鍵materialを複製しない。`RECONCILE_COPIES`限定serviceはlocal copy削除後だけmanifest evidenceへフォールバックする。evidence retention plannerは全entryをowner/mode/size/canonical hash込みでstrict列挙し、deletion resultとrequest hash・manifest identityを照合する。terminalかつattention不要のbundleだけを30日超過または11世代目以降の候補とし、protected、recovery-required、manifestだけの`orphan`、resultだけの`missing_manifest`は保持・表示する。この段階ではplanを返すだけでファイルを削除しない。永続evidence repositoryは最新resultとcleanup markerを安全に自動ロードし、古いsnapshotだけで変更retryを許可しない。
