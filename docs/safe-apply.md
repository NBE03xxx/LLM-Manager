...
`partial`/`failed`からcleanup Portへ進めるのは明示的な5分期限requestだけとする。requestはcleanup ID、source execution/request hash、backup、host/fingerprint、残存kindを自己hashで束縛する。serviceはexecution store全体のstrict列挙後にsourceと全bindingを再検証し、`completed`、改ざん、期限切れ、cancelではPortを呼ばない。現段階は認可・dispatch境界までであり、残存evidenceのcleanup実処理とcleanup結果永続化、production配置は未実装である。
