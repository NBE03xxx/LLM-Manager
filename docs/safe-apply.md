...
backup一覧はlocal manifest、remote retention record、直近local/remote retention evidence、検証済み削除resultをcopyごとにread-only集約する。local retention resultも削除前後の一覧差分、host、評価時刻、removed/remaining、partial/failed/unknown、errorをhashで束縛し、0700 directory内の0600 immutable fileへ保存する。片側削除失敗後のread-only reconciliation resultはsource deletion result hash、manifest hash、host/fingerprint、観測時刻、両copy presence/stateをcanonical hashで束縛し、0700/0600 immutable storeへ保存する。協調削除はremote mutation前にcanonical manifestをdeletion request hash名の0700/0600 immutable evidenceへ保存し、保存・再読込・binding検証失敗時は削除を開始しない。このevidenceはmanifest identity/path/item metadataだけで、backup本文や鍵materialを複製しない。`RECONCILE_COPIES`限定serviceは最新deletion resultとlive manifestを実行直前に再束縛し、local copy削除後だけmanifest evidenceへフォールバックして両copy observerと永続化runnerへ渡す。retention evidenceの再取得は`REFRESH_INVENTORY`へ分離する。永続evidence repositoryは再起動後に各private storeを安全に列挙し、最新retention/deletion/reconciliation resultとattempt cleanup markerを自動ロードする。live観測が片側でも不能な場合は保存済みsnapshotを表示へ使うが、古いsnapshotだけで変更retryを許可しない。未知entry、symlink、owner/mode、canonical hash、binding不一致はfail-closedに停止する。cleanup-only action serviceは元の変更を再実行せずuser staging cleanupとmarker保存だけを行う。
