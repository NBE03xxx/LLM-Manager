...
Backup一覧状態集約core実装: local manifest、remote retention record、local/remote retention run result、協調削除viewをbackup ID単位で集約し、copy presenceと`both_available/local_only/remote_only/both_deleted/unknown`、retention削除、cleanup pending、attentionを表示用immutable modelへ変換する。local retentionも削除前後をread-only再一覧してremoved/remainingを照合し、host/time/state/errorをcanonical hash付きresultへ束縛して0700/0600 local storeへimmutable保存する。永続evidence repositoryは再起動後にhost/fingerprintへ束縛された最新retention resultとbackupごとの最新deletion resultを安全に列挙し、対応attemptのcleanup markerまで検証して一覧へ自動ロードする。未知entry、symlink、owner/mode、canonical hash、binding不一致は一覧をfail-closedに停止する。cleanup-only action serviceは最新resultとpending状態を実行直前に再検証し、retention prune、remote delete、local delete、helper invokeを再実行せずuser staging cleanupだけを行う。再試行actionはevidenceに基づく限定操作だけとし、protected、remote-only、unknownでは変更actionを出さない。GUI接続はPhase 5で行う。
