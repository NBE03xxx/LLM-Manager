...
Dual-copy協調削除core実装: 5分期限のrequestへbackup/host/fingerprint/manifest hashを束縛し、remoteを先、localを後の固定順序で削除する。remote失敗・不明時はlocalを未実行のまま保持し、remote成功後のlocal失敗も補償削除や再作成を行わない。各copyの`deleted/already_absent/failed/not_attempted/unknown`、error code、read-only再観測結果、`both_available/local_only/remote_only/both_deleted/unknown`表示状態をhash付きimmutable resultへ記録する。sandbox/fakeまで完了し、remote helper deletion protocol、実SSH、再試行UXは未実装である。
