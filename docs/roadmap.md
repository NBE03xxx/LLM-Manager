...
残作業: Secret Service実desktop positive Gate、PolicyKit positive、転送中の実SSH切断、remote journal取得、remote retention/deletion実機Gate、deb lifecycle Gate。Secret ServiceとPolicyKitはnegative availability Gateまで完了した。OpenSSH negative compatibility Gateに加え、disposable `llm-manager-gate`で事前導入済みremote helperのpositive compatibility、user staging、外部端末sudo、root-owned暗号化recovery copy、receipt再取得、cleanupまで完了した。recovery request identityはhelper起動前に0700/0600 immutable storeへ保存し、`$XDG_STATE_HOME/llm-manager/remote-recovery/attempts`へ固定した。2プロセス実Gateでは第1プロセス終了後にmanifest/attemptを再読込し、helperを再実行せず同一hashのreceiptを回収・検証・cleanupした。改ざん、衝突、未知entry、unsafe metadataは拒否する。local root変更のdaemon-reload/restartはsandbox fake workflowまで完了している。これらが完了するまでPhase 4 Exitは未達とする。
