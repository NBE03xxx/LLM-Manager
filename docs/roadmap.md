...
Backup一覧状態集約core実装: local manifest、remote retention record、local/remote retention run result、協調削除viewをbackup ID単位で集約し、copy presenceと`both_available/local_only/remote_only/both_deleted/unknown`、retention削除、cleanup pending、attentionを表示用immutable modelへ変換する。local retentionも削除前後をread-only再一覧してremoved/remainingを照合し、host/time/state/errorをcanonical hash付きresultへ束縛する。再試行actionはevidenceに基づき`recover_remote_result`、`retry_remote_delete`、`retry_local_delete`、`retry_staging_cleanup`、read-only `reconcile_copies`へ限定する。protected、remote-only、unknownでは根拠のない変更actionを出さず、自動削除・自動再作成もしない。cross-host/fingerprint、hash不正、重複IDを拒否する。GUI接続はPhase 5で行う。
