...
残作業: Secret Service実desktop positive Gate、remote復旧copy、PolicyKit positive/remote helperの実環境integration Gate、remote journalとのSSH切断統合。Secret Serviceはstable unavailableのnegative Gateまで完了した。PolicyKitはGNOME sessionでauthority到達とCLI導入をread-only確認し、deb未installのため対象action/helperが存在しないnegative Gateまで完了した。local root変更のdaemon-reload/restartはsandbox fake workflowまで完了し、local helper同梱debはsandbox build/artifact検査まで完了した。実install、実PolicyKit認証、実systemd操作はdisposable OS Gateまで行わない。これらが完了するまでPhase 4 Exitは未達とする。
