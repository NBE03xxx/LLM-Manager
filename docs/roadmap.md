...
Dual-copy協調削除core/remote protocol実装: 5分期限のrequestへbackup/host/fingerprint/manifest hashを束縛し、remoteを先、localを後の固定順序で削除する。remote側は事前検証済みreceiptのhash、key reference、固定storage location、全item hashもcanonical protocolへ束縛し、user stagingのrequest ID/hashだけで限定`invoke-deletion`を起動する。root backendは保存済みreceipt/retentionを再照合し、protectedを拒否し、暗号化itemを復号・hash検証してから削除する。remote失敗・不明時はlocalを未実行のまま保持し、各copy outcomeとread-only再観測状態をimmutable resultへ記録する。fake OpenSSH/root backendまで完了し、実SSH、再試行UXは未実装である。
