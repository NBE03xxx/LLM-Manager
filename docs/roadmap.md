...
残作業: Secret Service実desktop positive Gate、PolicyKit positive、remote journal/receiptの実SSH切断統合、remote retention/deletion実機Gate、deb lifecycle Gate。Secret ServiceとPolicyKitはnegative availability Gateまで完了した。OpenSSH negative compatibility Gateに加え、disposable `llm-manager-gate`で事前導入済みremote helperのpositive compatibility、user staging、外部端末sudo、root-owned暗号化recovery copy、receipt再取得、cleanupまで完了した。recovery request identityはhelper起動前に0700/0600 immutable storeへ保存し、`$XDG_STATE_HOME/llm-manager/remote-recovery/attempts`へ固定した。再起動後はmanifest/host/fingerprintを再検証して同一receiptだけをread-only回収し、改ざん、衝突、未知entry、unsafe metadataを拒否する。local root変更のdaemon-reload/restartはsandbox fake workflowまで完了している。これらが完了するまでPhase 4 Exitは未達とする。
