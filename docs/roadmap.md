...
残作業: Secret Service実desktop positive Gate、PolicyKit positive、転送中の実SSH切断、remote journal取得、Debian 13とlocal packageのdeb lifecycle Gate。disposable `llm-manager-gate`ではpositive recovery、2プロセスreceipt回収、retention/deletionに加え、remote helper debの同一版reinstall/remove/purge/reinstallを完了した。package不在時はcompatibilityが`missing`へfail closedし、dpkg管理外root backup/keyは全段階で保持、再install後は最新runtime hashと`ready`へ復帰した。recovery request/receiptは`$XDG_STATE_HOME/llm-manager/remote-recovery/{attempts,receipts}`へ0700/0600 immutable保存する。local root変更のdaemon-reload/restartはsandbox fake workflowまで完了している。これらが完了するまでPhase 4 Exitは未達とする。
