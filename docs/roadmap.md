...
残作業: Secret Service実desktop positive Gate、PolicyKit positive、Debian 13とlocal packageのdeb lifecycle Gate。disposable `llm-manager-gate`ではpositive recovery、2プロセスreceipt回収、retention/deletion、remote helper deb lifecycle、転送中の実SSH切断、root journal evidence取得とread-only reconciliationを完了した。切断時は`remote_staging_failed`、request/result未公開、root helper未起動、local正本保持、user staging cleanupを確認した。journal Gateでは固定passwordless read commandだけを一時許可し、canonical root evidenceのbinding検証後にremote targetを`unapplied`と判定した。Gate後はroot evidenceと一時sudoersを削除し、`sudo -n`拒否へ戻した。package不在時はcompatibilityが`missing`へfail closedし、dpkg管理外root backup/keyは全段階で保持、再install後は最新runtime hashと`ready`へ復帰した。recovery request/receiptは`$XDG_STATE_HOME/llm-manager/remote-recovery/{attempts,receipts}`へ0700/0600 immutable保存する。local root変更のdaemon-reload/restartはsandbox fake workflowまで完了している。これらが完了するまでPhase 4 Exitは未達とする。
...
User-only SSH staging具体化: remote userの固定相対root `.local/state/llm-manager/remote-helper`配下へ、request ID/request hashから操作directoryを導出する。既存itemをhash付き固定名で先に0600転送し、canonical requestを最後に公開してからrequest ID/hashだけで限定helperを起動し、固定`result.json`を1 MiB上限で取得する。期限・改ざん・item hash・cancelをhelper起動前にも検査する。system `ssh`/`scp`の固定argv、OpenSSH alias/ControlPath、0600 local一時file、bounded downloadを実装し、root helper起動はpasswordless/外部端末sudo専用invokerへ分離した。実SSH staging/result回収/cleanupは完了し、転送中の実切断Gateが残る。
