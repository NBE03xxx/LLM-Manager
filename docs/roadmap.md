...
先行実装: Qt非依存のpresentation layerとして6工程の`GuiStep`、workflow status、immutable `GuiState`、`GuiPresenter`を追加した。host未選択、二重診断、report host不一致を拒否し、complete/partial/failed、cancel request、host/plan変更時の承認失効をheadless testで検証する。ja/en catalogは同一key集合を持ち、localeのlanguage部分を選択し、未対応・空localeを英語へfallbackする。QThreadPool境界は`QRunnable`、QObject signal、CancellationToken、host単位active lockを持つoptional adapterとして追加し、PySide6不在時はstable `pyside6_unavailable`で停止する。system PythonにPySide6は未導入のため、制約に従い依存追加せず、実signal/event-loopとwidget Gateは後続とする。
