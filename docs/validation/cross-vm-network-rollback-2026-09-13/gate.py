...
ROOT=Path('/tmp/phase6-cross-vm-network-rollback-20260913/evidence')
...
RELAY_PATH='/tmp/phase6-cross-vm-network-rollback-20260913/hold/helper.py'
...
candidate=HostCandidate('ssh:phase6-cross-vm-network-rollback',HostKind.SSH,'Disposable Ubuntu SSH','phase6-cross-vm-network-rollback')
...
import subprocess
def remote_bytes():
    p=subprocess.run(['ssh','-o','BatchMode=yes','phase6-cross-vm-network-rollback',"python3 -c \"from pathlib import Path; p=Path('/home/yoshimi/.config/opencode/opencode.jsonc'); print(p.read_bytes().hex() if p.exists() else 'ABSENT')\""],capture_output=True,text=True,check=True,timeout=10)
...
plan=OptimizationPlan('phase6-'+MODE,report.report_id,stable_hash(report),
    ...
    EncryptionInfo(True,'AES-256-GCM',1,'phase6-cross-vm-network-rollback-20260913','local_secret_service'))
