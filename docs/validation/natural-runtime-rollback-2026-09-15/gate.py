 ...
 def observe(self):
  ...
  if outcome is not None and not state.busy and self._active_host_id is None:
   ...
   result={'status':outcome.status.value,'error':outcome.error,
     ...
     'plan_injected':False,'approval_injected':False,'transport_injected':False,'validation_injected':False,'external_runtime_outage':True,'transport_events':transport_events}
