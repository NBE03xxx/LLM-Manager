...
| PySide6 | Python 3.14: 6.8.6以上。Debian 13 system Python 3.13: 6.8.2.1以上 | GUI | required | OS/Python組合せごとのimport/Qt plugin/QThreadPool/i18n test |
...
PySide6は公式release note上6.8.6からPython 3.14対応が明記されるため、Python 3.14環境では最低版6.8.6とする。Debian 13のsystem packageはPython 3.13.5とPySide6 6.8.2.1の組合せであり、正式debではこのOS提供組合せだけを別のsupported minimumとして実機Gateする。配布時は検証済みのOS/Python/PySide6組合せへ制約し、未検証の新minorへ自動追随しない。
