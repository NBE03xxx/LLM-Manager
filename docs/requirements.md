...
- **対応環境:** Ubuntu 26.04とDebian 13をMVP正式対象とする。Python 3.14.4、PySide6 6.8.6以上、Ollama 0.33.2、OpenCode 1.18.25を初期検証基準とする。application/runtimeのsupported minimumはDebian 13 stockに合わせPython 3.13、PySide6 6.8.2.1（Debian system packageとPython 3.13の組合せに限定）、cryptography 43.0.0、SecretStorage 3.3.3とする。Python 3.14環境のPySide6最低版は6.8.6を維持する。配布時のPySide6はOS/Python組合せごとの検証済みversionへpinする。周辺バージョンは互換性fixtureと契約テストを通過後にversion matrixへ追加する。
