...
| FR-APPLY-01 | ADR-0004, allowlist | `AtomicExecutorTests`, `CoordinatorTests`, `HelperProtocolTests`, `HelperStagingTests`（path/symlink/stale/write failure、canonical request/hash/expiry/operation/path/unit allowlist、derived staging/owner/mode/mutation）、root helper/restart統合待ち | Phase 4一部完了 |
