...
| AC-09/10 | ADR-0003, architecture | `ArchitectureTests`（coreからUIへの逆依存なし）、`GuiPresenterTests`（二重開始/cancel/state transition、ChangeSet生成busy/success/failure）、`QtWorkerBoundaryTests`、`QtWindowBoundaryTests`、`QtRuntimeTests`。Ubuntu 26.04/PySide6 6.10.2で別thread、event-loop sentinel、result/cancel、Diagnose→Recommendations→ChangeSet Review vertical slice完了 | Phase 5一部完了 |
