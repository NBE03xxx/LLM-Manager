...
| FR-VALIDATE-01 | allowlist, version matrix | `CoordinatorTests`（file hash）、service/API/config validation待ち | Phase 4一部完了 |
| FR-ROLLBACK-01 | ADR-0005, threat model | `CoordinatorTests`（逆順restore/recovery required）、disconnect待ち | Phase 4一部完了 |
| FR-AUDIT-01 | threat model | redaction/journal/replay tests | Phase 4 |
