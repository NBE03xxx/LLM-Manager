...
| AC-09/10 | ADR-0003, architecture | `ArchitectureTests`（coreからUIへの逆依存なし）、`GuiPresenterTests`（二重開始/cancel/state transition）、`QtWorkerBoundaryTests`（optional PySide6境界、QRunnable/QThreadPool/signal契約、UI layer限定）。実signal/event-loop Gateは後続 | Phase 5一部完了 |
