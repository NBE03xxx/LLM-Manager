...
| FR-BACKUP-01 | ADR-0005, threat model | `LocalBackupStoreTests`, `BackupCryptoTests`, `DualCopyPrivilegedBackupStoreTests`, `RemoteHelperRecoveryCopyStoreTests`（local integrity/permission/restart reload/retention/protection、AES-GCM、local+remote receipt identity/item hash/key scope束縛、canonical限定transport request/receipt、転送/暗号化/receipt取得失敗、再接続改ざん、片側失敗時Apply停止、sandbox remote-root独立暗号化・permission・再読込・改ざん/symlink拒否）、実SSH/helper・remote retention待ち | Phase 4一部完了 |
