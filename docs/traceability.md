...
| FR-BACKUP-01 | ADR-0005, threat model | `LocalBackupStoreTests`, `BackupCryptoTests`, `DualCopyPrivilegedBackupStoreTests`（local integrity/permission/restart reload/retention/protection、AES-GCM、local+remote receipt identity/item hash/key scope束縛、片側失敗時Apply停止、sandbox remote-root独立暗号化・permission・再読込・改ざん/symlink拒否）、SSH転送・実remote helper/retention待ち | Phase 4一部完了 |
