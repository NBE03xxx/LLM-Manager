...
| FR-BACKUP-01 | ADR-0005, threat model | `LocalBackupStoreTests`, `BackupCryptoTests`, `DualCopyPrivilegedBackupStoreTests`, `RemoteHelperRecoveryCopyStoreTests`, `UserOnlySshRecoveryTransportTests`, `DualCopyDeletionReconcilerTests`（local integrity/permission/restart reload/retention/protection、AES-GCM、local+remote receipt identity/item hash/key scope束縛、canonical限定transport request/receipt、固定user staging・request-last公開・bounded result、転送/暗号化/receipt取得失敗、cancel/期限/再接続改ざん、片側失敗時Apply停止、sandbox remote-root独立暗号化・30日/10世代/protected/最後の1件保持・permission・再読込・改ざん/未知entry/symlink拒否、削除後both/local-only/remote-only/deleted/unknown照合）、実OpenSSH/helper retention・協調削除command待ち | Phase 4一部完了 |
