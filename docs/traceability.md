...
| FR-PLAN-01..02 | allowlist, safe-apply, gui | `OpenCodePlannerTests`, `OllamaDropInPlannerTests`（root helper capability Gateを含む）、`RecommendationPresentationTests`/`QtRuntimeTests`（actionable選択、selected ID binding、preview-only Review）。実ChangeSet GUI接続は後続 | Phase 3 core完了/Phase 4 hardening/Phase 5一部 |
